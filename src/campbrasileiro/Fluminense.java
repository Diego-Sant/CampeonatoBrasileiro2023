...
public class Fluminense extends JFrame implements ActionListener {
	...
	Fluminense() {
		...
		ImageIcon img1 = new ImageIcon(ClassLoader.getSystemResource("icons/fluminense.png"));
		...
		jp.setBorder(new TitledBorder(new LineBorder(new Color(98, 0, 238), 2), // ((r: g: b:), thickness)
				"Fluminense", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(55, 0, 179)));
		add(jp);
		
		try {
			String query = "SELECT * FROM jogador WHERE time= 'Fluminense'";
			Conn c = new Conn();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Fluminense frame = new Fluminense();
