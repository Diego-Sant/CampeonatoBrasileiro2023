...
public class AmericaMineiro extends JFrame implements ActionListener {
	...
	AmericaMineiro() {
		super("América Mineiro");
		
		ImageIcon img1 = new ImageIcon(ClassLoader.getSystemResource("icons/americamg.png"));
		Image img2 = img1.getImage().getScaledInstance(200, 230, Image.SCALE_DEFAULT);
		...
		jp.setBorder(new TitledBorder(new LineBorder(new Color(98, 0, 238), 2), // ((r: g: b:), thickness)
				"América Mineiro", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(55, 0, 179)));
		add(jp);
		
		try {
			String query = "SELECT * FROM jogador WHERE time= 'América Mineiro'";
			Conn c = new Conn();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AmericaMineiro frame = new AmericaMineiro();
