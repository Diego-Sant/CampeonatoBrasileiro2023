...
public class AtleticoMineiro extends JFrame implements ActionListener {
	...
	AtleticoMineiro() {
		super("Athlético Paranaense");
		
		ImageIcon img1 = new ImageIcon(ClassLoader.getSystemResource("icons/atleticomg.png"));
		...
		jp.setBorder(new TitledBorder(new LineBorder(new Color(98, 0, 238), 2), // ((r: g: b:), thickness)
				"Athlético Paranaense", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(55, 0, 179)));
		add(jp);
		
		try {
			String query = "SELECT * FROM jogador WHERE time= 'Atlético Mineiro'";
			Conn c = new Conn();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AtleticoMineiro frame = new AtleticoMineiro();
