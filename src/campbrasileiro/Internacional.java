...
public class Internacional extends JFrame implements ActionListener {
	...
	Internacional() {
		...
		ImageIcon img1 = new ImageIcon(ClassLoader.getSystemResource("icons/internacional.png"));
		...
		jp.setBorder(new TitledBorder(new LineBorder(new Color(98, 0, 238), 2), // ((r: g: b:), thickness)
				"Internacional", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(55, 0, 179)));
		add(jp);
		
		try {
			String query = "SELECT * FROM jogador WHERE time= 'Internacional'";
			Conn c = new Conn();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Internacional frame = new Internacional();
