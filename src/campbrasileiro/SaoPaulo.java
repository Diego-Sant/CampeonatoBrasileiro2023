...
public class SaoPaulo extends JFrame implements ActionListener {
	...
	SaoPaulo() {
		...
		ImageIcon img1 = new ImageIcon(ClassLoader.getSystemResource("icons/saopaulo.png"));
		Image img2 = img1.getImage().getScaledInstance(200, 210, Image.SCALE_DEFAULT);
		...
		jp.setBorder(new TitledBorder(new LineBorder(new Color(98, 0, 238), 2), // ((r: g: b:), thickness)
				"São Paulo", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(55, 0, 179)));
		add(jp);
		
		try {
			String query = "SELECT * FROM jogador WHERE time= 'São Paulo'";
			Conn c = new Conn();
